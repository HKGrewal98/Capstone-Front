  ...
  useEffect(()=>{
    ...
    myHeaders.append('Access-Control-Allow-Credentials', true)
      ...
      .then(function (response) {
        // console.log("Response in Supporting Documents",response.data);
...
<table className="table customTableMArgin">
  ...
  <tbody>
      ...
      <td>{SupportingDocumentsData?.project?.project_number}</td>
      ...
      <td>
      <svg className='m-1' width="20" height="17" viewBox="0 0 20 17" fill="none" xmlns="http://www.w3.org/2000/svg" style={{cursor:"pointer"}}  onClick={()=>{
            var myHeaders = new Headers();
            myHeaders.append("Content-Type", "application/json");
            myHeaders.append('Access-Control-Allow-Origin', 'http://localhost:8081')
            myHeaders.append('Access-Control-Allow-Credentials', true)
              ...
              axios({
                method: 'get',
                maxBodyLength: Infinity,
                url: `http://localhost:8081/report/download/${data?.file_id}`,
                headers:myHeaders,
                credentials: "include", 
                withCredentials:true,
